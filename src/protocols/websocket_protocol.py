...
def _resolve_websocket_header_kwarg() -> str:
    """返回当前 websockets.connect 支持的 headers 参数名。"""
    try:
        connect_signature = inspect.signature(websockets.connect)
        parameters = connect_signature.parameters
...
class WebsocketProtocol(Protocol):
    def __init__(self):
        ...
        self._header_kwarg_name = _resolve_websocket_header_kwarg()
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            connect_kwargs = {
                ...
                self._header_kwarg_name: self.HEADERS,
                "open_timeout": 12,
            ...
            logger.error(
                f"WebSocket连接超时: url={self.WEBSOCKET_URL}, type={type(e).__name__}, detail={repr(e)}"
            )
            await self._cleanup_connection()
            if self._on_network_error:
                self._on_network_error("无法连接服务: 连接超时")
            return False
        except Exception as e:
            logger.error(
                f"WebSocket连接失败: url={self.WEBSOCKET_URL}, type={type(e).__name__}, detail={repr(e)}",
                exc_info=True,
            )
            await self._cleanup_connection()
            if self._on_network_error:
                error_message = str(e).strip() or repr(e)
                self._on_network_error(f"无法连接服务: {error_message}")
            return False

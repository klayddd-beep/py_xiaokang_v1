...
class Application:
    ...
    def __init__(self):
        ...
        self._incoming_audio_queue: asyncio.Queue[bytes] | None = None
    ...
    async def run(self, *, protocol: str = "websocket", mode: str = "gui") -> int:
        ...
        try:
            ...
            self.spawn(self._incoming_audio_worker(), "audio:incoming_worker")
    ...
    def _initialize_async_objects(self) -> None:
        ...
        self._incoming_audio_queue = asyncio.Queue(maxsize=1000)
    ...
    def _on_incoming_audio(self, data: bytes):
        ...
        if not self._incoming_audio_queue:
            return
        try:
            self._incoming_audio_queue.put_nowait(data)
            ...
            try:
                ...
                logger.warning("收到音频过快，丢弃一帧")

    async def _incoming_audio_worker(self) -> None:
        """
        顺序处理服务端音频帧，避免爆发式发送时每帧各自建任务导致乱序或抢跑。
        """
        try:
            while self.running:
                if not self._incoming_audio_queue:
                    ...
                    continue
                data = await self._incoming_audio_queue.get()

...
class AudioCodec:
    ...
    async def _load_device_config(self):
        ...
        # 解析并校验设备（优先名称，避免热插拔后索引漂移）
        input_info = self._resolve_device_info(
            kind="input",
            configured_id=input_device_id,
            configured_name=input_device_name,
        )
        output_info = self._resolve_device_info(
            kind="output",
            configured_id=output_device_id,
            configured_name=output_device_name,
        )

        if not input_info or not output_info:
            logger.warning("配置的音频设备不可用，回退到自动检测")
            await self._auto_detect_devices()
            return

        # 从配置加载
        self.mic_device_id = input_info["index"]
        self.speaker_device_id = output_info["index"]

        requested_input_channels = int(audio_config.get("input_channels", 1))
        requested_output_channels = int(audio_config.get("output_channels", 1))

        max_input_channels = max(1, int(input_info["max_channels"]))
        max_output_channels = max(1, int(output_info["max_channels"]))

        self.input_channels = min(
            requested_input_channels, max_input_channels, AudioConfig.MAX_INPUT_CHANNELS
        )
        self.output_channels = min(
            requested_output_channels,
            max_output_channels,
            AudioConfig.MAX_OUTPUT_CHANNELS,
        )

        if self.input_channels != requested_input_channels:
            logger.warning(
                f"输入声道配置从 {requested_input_channels} 调整为 {self.input_channels}，"
                f"设备最大支持 {max_input_channels} 声道"
            )
        if self.output_channels != requested_output_channels:
            logger.warning(
                f"输出声道配置从 {requested_output_channels} 调整为 {self.output_channels}，"
                f"设备最大支持 {max_output_channels} 声道"
            )

        # 采样率以当前设备能力为准，避免热插拔后沿用旧配置导致噪声/失真
        self.device_input_sample_rate = int(
            input_info["sample_rate"] or AudioConfig.INPUT_SAMPLE_RATE
        )
        self.device_output_sample_rate = int(
            output_info["sample_rate"] or AudioConfig.OUTPUT_SAMPLE_RATE
        )
    ...
    def _resolve_device_info(self, kind: str, configured_id, configured_name):
        """
        解析设备信息（优先名称匹配，其次ID匹配），并校验方向声道可用.
        """
        try:
        ...
        def _pack(index: int):
            ...
            max_channels = int(device.get(channels_key, 0))
            if max_channels <= 0:
                return None
            return {
                "index": index,
        ...
        if configured_name:
            for index, device in enumerate(devices):
                if device.get("name") == configured_name:
                    info = _pack(index)
                    if info:
                        return info

            # 2) 名称包含匹配（兼容系统追加后缀）
            configured_name_low = str(configured_name).casefold()
            for index, device in enumerate(devices):
                device_name = str(device.get("name", ""))
                if configured_name_low in device_name.casefold():
                    info = _pack(index)
                    if info:
                        return info
        ...
        if isinstance(configured_id, int):
            info = _pack(configured_id)
            if info:
                return info

        # 4) 自动选择兜底
        auto_info = select_audio_device(kind, include_virtual=False)
        if auto_info:
            return _pack(int(auto_info["index"]))
    ...
    async def _create_resamplers(self):
        ...
        """
        input_resample_quality = self.config.get_config(
            "AUDIO_DEVICES.input_resample_quality", "MQ"
        )
        output_resample_quality = self.config.get_config(
            "AUDIO_DEVICES.output_resample_quality", "MQ"
        )
        ...
        if self.device_input_sample_rate != AudioConfig.INPUT_SAMPLE_RATE:
            self.input_resampler = soxr.ResampleStream(
                ...
                dtype="float32",
                quality=input_resample_quality,
            )
            logger.info(
                f"输入重采样: {self.device_input_sample_rate}Hz → 16kHz, 质量={input_resample_quality}"
            )
        ...
        if self.device_output_sample_rate != AudioConfig.OUTPUT_SAMPLE_RATE:
            self.output_resampler = soxr.ResampleStream(
                ...
                dtype="float32",
                quality=output_resample_quality,
            )
            logger.info(
                f"输出重采样: {AudioConfig.OUTPUT_SAMPLE_RATE}Hz → "
                f"{self.device_output_sample_rate}Hz, 质量={output_resample_quality}"
            )
    ...
    async def _create_streams(self):
        ...
        """
        try:
            stream_latency = self.config.get_config("AUDIO_DEVICES.stream_latency", "high")
            ...
            logger.info(
                ...
                f"输出: {self.device_output_sample_rate}Hz {self.output_channels}ch | 延迟={stream_latency}"
            )
    ...
    def _output_callback_direct(self, outdata, frames):
        ...
        try:
            ...
            if self._need_output_upmix:
                ...
                np.clip(multi_channel, -1.0, 1.0, out=multi_channel)
                outdata[:] = multi_channel
            else:
                # 单声道输出
                np.clip(mono_samples_float, -1.0, 1.0, out=mono_samples_float)
        ...
        try:
            ...
            if len(self._resample_output_buffer) >= frames:
                ...
                if self._need_output_upmix:
                    ...
                    # 单声道输出
                    np.clip(mono_data, -1.0, 1.0, out=mono_data)
                    outdata[:, 0] = mono_data
            else:
                outdata.fill(0)
                # 数据不足时输出静音
    ...
    async def pause_streams(self):
        """暂停并释放输入/输出流句柄（不销毁编解码器）"""
        if self._is_closing:
            return
        ...
        try:
            ...
            logger.info("音频流已暂停并释放设备占用")
        except Exception as e:
            logger.warning(f"暂停音频流失败: {e}")

    async def resume_streams(self):
        """恢复输入/输出流（若已释放）"""
        if self._is_closing:
            return

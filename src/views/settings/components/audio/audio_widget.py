...
class AudioWidget(QWidget):
    ...
    def _do_input_test(self, device_id):
        ...
        codec = None
        try:
            app, codec = self._get_runtime_audio_codec()
            self._pause_runtime_audio_streams(app, codec)
            ...
            if "device unavailable" in error_text or "paerrorcode -9985" in error_text:
                self._append_status_threadsafe(
                    "检测到设备被占用：请先停止正在运行的语音会话或重启应用后再测试。"
                )
            ...
            self._resume_runtime_audio_streams(app, codec)
            # 重置UI状态（切回主线程）
    ...
    def _get_runtime_audio_codec(self):
        """获取运行中的 AudioCodec 实例（若可用）"""
        try:
            ...
            self.logger.debug(f"获取运行时音频实例失败: {e}")
            return None, None

    def _pause_runtime_audio_streams(self, app, codec):
        """测试前暂停运行中音频流，避免设备占用冲突"""
        if not app or not codec or not app._main_loop:
            return

        try:
            future = asyncio.run_coroutine_threadsafe(codec.pause_streams(), app._main_loop)
            future.result(timeout=3)
            self._append_status_threadsafe("已临时暂停主音频流，开始设备测试")
        except Exception as e:
            self.logger.debug(f"暂停主音频流失败，继续测试: {e}")

    def _resume_runtime_audio_streams(self, app, codec):
        """测试后恢复运行中音频流"""
        if not app or not codec or not app._main_loop:
            return

        try:
            future = asyncio.run_coroutine_threadsafe(codec.resume_streams(), app._main_loop)
            future.result(timeout=5)
            self._append_status_threadsafe("设备测试完成，主音频流已恢复")
        except Exception as e:
            self.logger.warning(f"恢复主音频流失败: {e}")
